  ...
  if (::IsWindow(m_hParent)) {
    ...
    for (it=m_ctrls.begin(); it!=m_ctrls.end(); ++it) {
      ...
      if (hFix & kLeft) {
        ...
        currCtrlSize.left = ((hFix & kWidth) && (hFix & kRight)) ? static_cast<LONG>(it->m_origSize.left + currParentSize.Width() - m_origParentSize.Width()) : static_cast<LONG>(it->m_origSize.left * xRatio);
      }
      if (hFix & kRight) {
        currCtrlSize.right = it->m_origSize.right + currParentSize.Width() - m_origParentSize.Width();
      } else {
        currCtrlSize.right = (hFix & kWidth) ? static_cast<LONG>(currCtrlSize.left + it->m_origSize.Width()) : static_cast<LONG>(it->m_origSize.right * xRatio);
      }

      if (vFix & kTop) {
        currCtrlSize.top = it->m_origSize.top;
      } else {
        currCtrlSize.top = ((vFix & kHeight) && (vFix & kBottom)) ? static_cast<LONG>(it->m_origSize.top + currParentSize.Height() - m_origParentSize.Height()) : static_cast<LONG>(it->m_origSize.top * yRatio);
      }
      if (vFix & kBottom) {
        currCtrlSize.bottom = it->m_origSize.bottom + currParentSize.Height() - m_origParentSize.Height();
      } else {
        currCtrlSize.bottom = (vFix & kHeight) ? static_cast<LONG>(currCtrlSize.top + it->m_origSize.Height()) : static_cast<LONG>(it->m_origSize.bottom * yRatio);
      }
